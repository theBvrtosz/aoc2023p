...
class Day1(AocDay):
    ...
    def __remove_non_numerics_from_calibration_values(self, calibration_values):
        return (re.sub("[^0-9]", "", calibration_value) for calibration_value in calibration_values)
    
    def __extract_two_digit_from_calibration_values(self, calibration_values):
        return (int(f"{numeric_value[0]}{numeric_value[-1]}") for numeric_value in calibration_values if len(numeric_value) > 0 )

    def part1(self):
        extracted_numerics = self.__remove_non_numerics_from_calibration_values(self.input_lines)
    ...
    def part2(self):
        ...
        for corrupted_value in self.input_lines:
            ...
            for spelled_number in spelled_numbers.keys():
                if spelled_number in corrupted_value:
